...
| ------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | -------------------------------------------------------- | ---------------------------------------------------- |
| ODS 原始数据层                 | 存放原始数据，后端数据快照。<br />创建分区表                 |                                                              |                                                              |                                                          | 1.从业务库直接同步的数据<br />2.直接接入原始日志数据 |
| DWD 数据明细层<br />(原子指标) | 维度建模<br />一般是星型模型                                 | **业务过程**驱动，按照最细粒度指标存储<br />划分数据主题域，面向主题<br />定期同步 | 1.选择业务过程：履约主题域的到课参课<br />2.定义粒度：课程(小节)+用户<br />3.确认维度：学员信息；课程信息；时间信息；商品信息<br />4.确认事实：学习持续时间；进度完成情况；出勤情况 | 空值处理；验证业务数据有效性；规范数据格式；统一数据标准 | 事务性事实表，周期性快照事实宽表，累计快照事实宽表   |
...
| ADS 应用层                     |                                                              | 个性化指标业务                                               | 日活、月活、留存、留存率、转化、流失、复购、GMV、退货、退费、TOPN、热门数据 |                                                          |                                                      |
...
2. 周期快照事实表
   ...
   不保留所有数据，只保留固定时间间隔的数据。在固定时间周期内采集一次业务对象的状态指标存入事实表
...
4. 累计快照事实表
   ...
   跟踪业务事实的变化(生命周期)，每条记录代表一次业务过程。在业务过程进行中需要不断更新。
   
   **设计方法：** 创建每个阶段的关键时间戳、状态、累计指标；对比来源表，如果出现了新阶段，更新原记录的对应字段。
