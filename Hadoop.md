...
1. HDFS写入时，把文件分隔成block，每个block的3个副本存储在三个集群机器上。

```java
■ 第一副本 机架1：放置在上传文件的 DataNode上; 就近原则提高写入效率
■ 第二副本 机架2 DN1：放置在与第一个副本不同的机架的节点上;跨机架冗余，如果机架1宕机了，数据不会丢失
■ 第三副本 机架2 DN2：与第二个副本相同机架的不同节点上;降低带宽压力，适用于热点数据：
					一个客户端从 Rack1 的副本读取。
					第二个客户端可以从 Rack2 的副本读取，避免抢占带宽。
```

2. 针对DataNode宕机

   DataNode定期发送心跳信号给NN。如果一定时间内没有收到DN的心跳，认为宕机了。

   **处理：** NameNode从现有的副本中选择一个源节点，把这些数据开复制到其他健康datanode上

3. 针对网络故障

   ACK确认机制，没有收到ACK则认为网络故障

   **处理：** 1。客户端重试 2.客户端调整pipeline，选择新的datanode写入

4. 针对数据损坏(脏数据)

   传输数据的时候发送校验码，datanode定期向namenode发送数据块存储情况时会检测总和校验码。如果数据错误就不发送改数据块的信息
...
| 存储格式         | 描述                           | 原理                                                         | 优缺点                                                       |
| ---------------- | ------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **TextFile**     | 默认的纯文本格式。             | 易于使用和理解。                                             | 易于使用和理解。效率低，不支持压缩。                         |
| **Parquet**      | 流行的列式存储格式。           | 每列数据存储一个Column组。每列数据单独存储；每列数据使用字典编码压缩；列数据切分成很多pages | 结构轻便，查询的时候查询这一列的所有page。                   |
| **ORC**          | 优化的行列式存储格式。         | 把所有列划分Stripe(Block)；创建索引filter和min/max           | 查询的时候先根据filter索引判断stripe，再只读那块stripe。节约IO |
...
1. NameNode & SeconderyNameNode

   - fsimage：存储元数据
   - editlog：数据存入时，元数据信息追加到editlog中。

   editlog达到一定数量后，copy到SNN中合并fsimage，再更新NN的fsimage文件。一旦NN中数据丢失，SNN的数据作为备份协助恢复。
...
Reduce端`merge`：  Reducer 从多个 Mapper 拉取其对应分区的数据，把不同分区的数据`归并排序`

**作用：**

- 快排：保证Spill的临时文件内有序，只需要在内存实现排序
- 归并：多路有序的临时文件合并
- 归并：Reduce把不同分区的数据文件copy过来进行归并排序，多路有序文件的合并

### MapReduce的Combine

对MapTask的输出做一个重复key值的合并操作（{key，[V1,V2]}）

**好处：**局部汇总减少网络传输。Map 输出的数据先被合并 ⇒ 发送到 Reducer 的 key 数量变少

**使用限制：**加法、计数可以；最大值、求平均就不行

**和reduce区别：**

​	combine只能做局部合并、不能改变 key/value 格式

​	reduce取出一个一个的键值对group，调用reduce()方法计算
...
- 分区排序
- 不需要申请新的内存，始终使用这个内存空间，重复利用。

**原理：**

环形缓冲区是数组， `equator赤道`把key/value实际数据(顺时针) 和 元数据(逆时针) 分开 `value起始位置+ key起始位置 + partition + value的长度`

当 Map 端的环形缓冲区使用率超过 80%（`io.sort.spill.percent`），Hadoop 会自动触发一个 **异步 spill 操作** 。
